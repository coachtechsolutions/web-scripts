let pkIntervalTimer = setInterval(myTimer, 200);
        ...
        try {
            ...
            if (appointmentWidgets) {
                ...
                if (whichPage.length > 0) {
                    ...
                    if (qNode.length>0) {
                        qNode[0].childNodes[0].childNodes[0].nodeValue=' Zvolit'; //Select
                    }
                    ...
                    if (qNode.length>0){
                        qNode[0].childNodes[0].nodeValue='Tento měsíc není žádný volný termín'; //No free slots this month
                        qNode[0].childNodes[1].childNodes[0].nodeValue='Najít další volný termín'; //Find next free slot
                    }
                ...
                if (whichPage.length > 0){
                    if(document.getElementById('form-builder')){
                        ...
                        //document.querySelector('textarea').attributes.placeholder.value='Co bych měla vědět ještě před schůzkou?'; //What should I know before...
                        document.querySelector('h4.text-info').innerText='Prosím, vyplňte informace'; //Please fill in information
                    }
                    updateBookingInfoValues();
                ...
                if (whichPage.length > 0){
                    document.querySelector('h5.confirmation-message').innerText='Vaše konzultace je naplánovaná'; // Your session is planned
                    updateBookingInfo();
                    updateBookingInfoValues();
                    ...
                    if (qNode.length>0){
                        qNode[0].childNodes[0].nodeValue=" Další..."; //Next...
                    }

                }
                qNode=document.querySelectorAll('button.btn-schedule');
                if (qNode.length>0){
                    qNode[0].childNodes[0].nodeValue= 'Naplánovat schůzku' //Book the session
                }
            }
        } catch (e) {
    ...
    function updateDateSelectors() {
        ...
        if (selDateTime){
            selDateTime.childNodes[0].nodeValue='Vyberte datum a čas'; //Select date and time
        }
        ...
        document.querySelector('div.timezone-label').innerText='Časová zóna'; //Time zone
